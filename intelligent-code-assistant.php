		...
		array(
			...
			'input_schema'        => array(
				...
				'properties' => array(
					'code' => array(
						'type'        => 'string',
						'description' => __( 'The raw code snippet content to analyze.', 'intelligent-code-assistant' ),
						'minLength'   => 1,
					),
				),
				'required'             => array( 'code' ),
...
{$code}";
		...
		if ( ( str_starts_with( $trimmed_code, '{' ) && str_ends_with( $trimmed_code, '}' ) ) || 
			...
			return array(
				...
				'title'           => __( 'JSON Structure', 'intelligent-code-assistant' ),
				'highlightLines'  => '1',
		...
		if ( preg_match( '/^<[!a-zA-Z]/', $trimmed_code ) ) {
			return array(
				'codeLanguage'    => 'HTML',
				'filename'        => 'index.html',
				'title'           => __( 'HTML Markup', 'intelligent-code-assistant' ),
				'highlightLines'  => '',
				'showLineNumbers' => $lines_count > 3,
			);
		}
		...
		if ( str_contains( $trimmed_code, '<?php' ) || str_contains( $trimmed_code, 'namespace ' ) ) {
			return array(
				...
				'title'           => __( 'PHP Script', 'intelligent-code-assistant' ),
				'highlightLines'  => '',
		...
		if ( preg_match( '/(const|let|var|import|export|function)\s/', $trimmed_code ) ) {
			return array(
				'codeLanguage'    => 'JS',
				'filename'        => 'script.js',
				'title'           => __( 'JavaScript Code', 'intelligent-code-assistant' ),
				'highlightLines'  => '',
				'showLineNumbers' => $lines_count > 3,
			);
		}
		...
		return array(
			...
			'title'           => __( 'Code Snippet', 'intelligent-code-assistant' ),
			'highlightLines'  => '',
			'showLineNumbers' => $lines_count > 3,
		);
	}
	...
	register_rest_route(
		...
		array(
			...
			'callback'            => function( WP_REST_Request $request ) {
				...
				if ( ! $user_id ) {
					return new WP_Error( 'unauthorized', __( 'User not logged in.', 'intelligent-code-assistant' ), array( 'status' => 401 ) );
				}

				if ( empty( $block_id ) ) {
					return new WP_Error( 'invalid_id', __( 'Block ID is required.', 'intelligent-code-assistant' ), array( 'status' => 400 ) );
				}
